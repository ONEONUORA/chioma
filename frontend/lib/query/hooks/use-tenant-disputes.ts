...
function matchesFilter(
  dispute: TenantDisputeRecord,
  filters: TenantDisputeFilters,
): boolean {
  if (
    filters.status &&
    filters.status !== 'ALL' &&
    ...
    queryFn: async () => {
      try {
        ...
        const responseData = await apiClient.get<{
          data?: TenantDisputeRecord[] | { disputes?: TenantDisputeRecord[] };
        }>(`/disputes?${params}`);
        const apiData = responseData.data;
        // Normalize API response to TenantDisputeRecord (similar to admin hook)
        const disputes: TenantDisputeRecord[] = (
          (apiData?.data as TenantDisputeRecord[] | undefined) ||
          (apiData as { disputes?: TenantDisputeRecord[] })?.disputes ||
          []
        ).map((d: TenantDisputeRecord) => ({
          id: String(d.id),
